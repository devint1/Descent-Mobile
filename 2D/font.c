...
int gr_internal_string_clipped(int x, int y, unsigned char *s );
int gr_internal_string_clipped_m(int x, int y, unsigned char *s );
extern ubyte *gr_bitblt_fade_table;

unsigned char *find_kern_entry(grs_font *font,char first,char second)
{
...
int gr_internal_string2(int x, int y, unsigned char *s )
{
	unsigned char * fp;
	unsigned char * text_ptr, * next_row, * text_ptr1;
	int r, BitMask, i, bits, width, spacing, letter, underline;
	...
	VideoOffset1 = (unsigned long)DATA + y * ROWSIZE + x;
...
int gr_internal_string2m(int x, int y, unsigned char *s )
{
	unsigned char * fp;
	unsigned char * text_ptr, * next_row, * text_ptr1;
	int r, BitMask, i, bits, width, spacing, letter, underline;
...
int gr_internal_color_string(int x, int y, unsigned char *s )
{
	unsigned char * fp;
	unsigned char * text_ptr, * next_row, * text_ptr1;
...
void gr_scale_printf(int x, int y, fix scale_x, fix scale_y, char * format, ... ) {
	...
	va_start(args, format);
	vsprintf(buffer, format, args);
	gr_scale_string(x, y, scale_x, scale_y, buffer);
}
...
void gr_set_curfont( grs_font * new_font )
{
	FONT = new_font;
}
