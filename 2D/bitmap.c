...
{
    grs_bitmap *new_bitmap;

    new_bitmap = (grs_bitmap *)malloc( sizeof(grs_bitmap) );
	new_bitmap->bm_x = 0;
	new_bitmap->bm_y = 0;
    new_bitmap->bm_w = w;
    new_bitmap->bm_h = h;
	new_bitmap->bm_type = 0;
	new_bitmap->bm_flags = 0;
...
grs_bitmap *gr_create_sub_bitmap(grs_bitmap *bm, int x, int y, int w, int h )
{
    ...
    new_bitmap = (grs_bitmap *)malloc( sizeof(grs_bitmap) );
	new_bitmap->bm_x = x+bm->bm_x;
	new_bitmap->bm_y = y+bm->bm_y;
	new_bitmap->bm_w = w;
    new_bitmap->bm_h = h;
	new_bitmap->bm_flags = bm->bm_flags;
	new_bitmap->bm_type = bm->bm_type;
