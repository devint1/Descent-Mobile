...
{
	unsigned char * data;
	grs_canvas *new_canvas;
	
	new_canvas = (grs_canvas *)malloc( sizeof(grs_canvas) );
	data = (unsigned char *)malloc(w*h);

	new_canvas->cv_bitmap.bm_x = 0;
	new_canvas->cv_bitmap.bm_y = 0;
	new_canvas->cv_bitmap.bm_w = w;
	new_canvas->cv_bitmap.bm_h = h;
	new_canvas->cv_bitmap.bm_flags = 0;
	...
	new_canvas->cv_font = NULL;
	new_canvas->cv_font_fg_color = 0;
	new_canvas->cv_font_bg_color = 0;
	return new_canvas;
}
...
grs_canvas *gr_create_sub_canvas(grs_canvas *canv, int x, int y, int w, int h)
{
	...
	new_canvas->cv_bitmap.bm_x = x+canv->cv_bitmap.bm_x;
	new_canvas->cv_bitmap.bm_y = y+canv->cv_bitmap.bm_y;
	new_canvas->cv_bitmap.bm_w = w;
	new_canvas->cv_bitmap.bm_h = h;
	new_canvas->cv_bitmap.bm_flags = 0;
   ...
   new_canvas->cv_font = canv->cv_font;
	new_canvas->cv_font_fg_color = canv->cv_font_fg_color;
	new_canvas->cv_font_bg_color = canv->cv_font_bg_color;
   return new_canvas;
}
...
void gr_init_sub_canvas(grs_canvas *new_canvas, grs_canvas *src, int x, int y, int w, int h)
{
	new_canvas->cv_color = src->cv_color;
	new_canvas->cv_drawmode = src->cv_drawmode;
	new_canvas->cv_font = src->cv_font;
	new_canvas->cv_font_fg_color = src->cv_font_fg_color;
	new_canvas->cv_font_bg_color = src->cv_font_bg_color;

	new_canvas->cv_bitmap.bm_x = src->cv_bitmap.bm_x+x;
	new_canvas->cv_bitmap.bm_y = src->cv_bitmap.bm_y+y;
	new_canvas->cv_bitmap.bm_w = w;
	new_canvas->cv_bitmap.bm_h = h;
	new_canvas->cv_bitmap.bm_flags = 0;
	new_canvas->cv_bitmap.bm_type = src->cv_bitmap.bm_type;
	new_canvas->cv_bitmap.bm_rowsize = src->cv_bitmap.bm_rowsize;


	new_canvas->cv_bitmap.bm_data = src->cv_bitmap.bm_data;
	new_canvas->cv_bitmap.bm_data += y*src->cv_bitmap.bm_rowsize;
	new_canvas->cv_bitmap.bm_data += x;
}
