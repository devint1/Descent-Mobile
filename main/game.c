...
// Reworked a bunch of the screen rez stuff to support arbitrary resolutions
...
#define LETTERBOX_HEIGHT grd_curscreen->sc_h * 0.85
...
void update_cockpits(int force_redraw)
{
	...
	grs_point bar_scale_pts[] = {
		{ 0, i2f(max_window_h) },
		{ i2f(320), i2f(max_window_h) + i2f(53) },
		{ i2f(grd_curscreen->sc_w), i2f(max_window_h) + 53 * scale_y }
	};
	...
	case CM_STATUS_BAR:
		gr_set_current_canvas(&VR_screen_pages[VR_current_page]);
		PIGGY_PAGE_IN(cockpit_bitmap[Cockpit_mode]);
		scale_bitmap(&GameBitmaps[cockpit_bitmap[Cockpit_mode].index], bar_scale_pts);
		w = Game_window_w;
		h = Game_window_h;
...
//		if ( VR_use_paging )	gr_show_canvas( &VR_screen_pages[VR_current_page] );
	...
	case SCREEN_MENU:
		...
		gr_init_sub_canvas( &VR_screen_pages[0], &grd_curscreen->sc_canvas, 0, 0, grd_curscreen->sc_w / 2, grd_curscreen->sc_h / 2);
	...
	case SCREEN_GAME:
		...
		if ( VR_render_mode == VR_NONE )	{
			if ( max_window_h == 0 )	{
				max_window_h = grd_curscreen->sc_h - f2fl(scale_y) * GameBitmaps[cockpit_bitmap[CM_STATUS_BAR].index].bm_h;
				Game_window_h = max_window_h;
			} else {
				max_window_h = grd_curscreen->sc_h - f2fl(scale_y) * GameBitmaps[cockpit_bitmap[CM_STATUS_BAR].index].bm_h;
			}
...
void render_countdown_gauge()
{
	...
	fix scale_y = fixdiv(i2f(grd_curscreen->sc_h), i2f(200));
	fix scale_factor = fmin(scale_x, scale_y);
	if (!Endlevel_sequence && Fuelcen_control_center_destroyed  && (Fuelcen_seconds_left>-1) && (Fuelcen_seconds_left<127))	{
		...
		y = 20 * f2i(scale_factor);
		if (!((Cockpit_mode == CM_STATUS_BAR) && (Game_window_w >= 19)))
			y += 5;

		gr_scale_printf(0x8000, y, scale_factor, scale_factor, "T-%d s", Fuelcen_seconds_left );
	}
}
...
void diminish_palette_towards_normal(void)
{
	...
	if (FrameTime < F1_0/DIMINISH_RATE) {
		if (drand() < FrameTime*DIMINISH_RATE/2)	//	Note: rand() is in 0..32767, and 8 Hz means decrement every frame
			dec_amount = 1;
	} else {
...
{	
	...
	bg.bmp = gr_create_bitmap( w+30*f2fl(Scale_factor), h+30*f2fl(Scale_factor) );
	gr_bm_ubitblt(w+30*f2fl(Scale_factor), h+30*f2fl(Scale_factor), 0, 0, x-15*f2fl(Scale_factor), y-15*f2fl(Scale_factor), &(grd_curscreen->sc_canvas.cv_bitmap), bg.bmp );

	nm_draw_background(x-15*f2fl(Scale_factor),y-15*f2fl(Scale_factor),x+w+15*f2fl(Scale_factor)-1,y+h+15*f2fl(Scale_factor)-1);

	gr_set_fontcolor( gr_getcolor(31, 31, 31), -1 );
	gr_scale_string( 0x8000, y, Scale_factor, Scale_factor, msg );
}
...
void ReadControls() {
	...
	if (Player_exploded) { //Player_is_dead && (ConsoleObject->flags & OF_EXPLODING) ) {
		if (exploding_flag == 0)	{
			...
			for (i = 0; i<3; i++)
				if (mouse_button_down_count(i, &mouse_x, &mouse_y)>0) Death_sequence_aborted = 1;
			for (i = 0; i<256; i++)
...
void close_game() 
{
	...
	if(gr_video_memory) {
		free(gr_video_memory);
		gr_video_memory = NULL;
	}
	
	clear_warn_func(game_show_warning);     //don't use this func anymore

	Function_mode = FMODE_NORENDER;
}
