	...
	if (HUD_nmessages > 0 )	{
		...
		if ((Cockpit_mode == CM_STATUS_BAR) && (VR_render_sub_buffer[0].cv_bitmap.bm_y >= 19)) {
			...
			if (strcmp(Displayed_background_message, message)) {
				...
				ycrd = grd_curcanv->cv_bitmap.bm_y - 9 * f2fl(scale_factor);
				...
				gr_scale_printf((grd_curcanv->cv_bitmap.bm_w-w)/2, ycrd, scale_factor, scale_factor, message );
			...
			y = 3;
		  	...
		  	for (i=0; i<HUD_nmessages; i++ )	{	
				...
				h *= f2i(scale_factor);
				gr_set_fontcolor( HUD_color, -1);
				gr_scale_printf((grd_curcanv->cv_bitmap.bm_w-w)/2,y,scale_factor,scale_factor, &HUD_messages[n][0] );
				y += h+f2i(scale_factor);
			}
		}
	}
...
void player_dead_message(void)
{
	...
	if (!Arcade_mode && Player_exploded) { //(ConsoleObject->flags & OF_EXPLODING)) {
		...
		gr_scale_printf(0x8000, grd_curcanv->cv_bitmap.bm_h-8*f2i(scale_factor), scale_factor, scale_factor, TXT_PRESS_ANY_KEY);
		gr_set_curfont( GAME_FONT );    
	}
