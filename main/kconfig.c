...
void kc_drawitem(kc_item *item, int is_current)
{
	...
	gr_scale_string(item->x * f2fl(Scale_x), item->y * f2fl(Scale_y), Scale_factor, Scale_factor, Text_string[item->text_num1]);
	...
	gr_urect(f2fl(Scale_x) * (item->w1 + item->x), f2fl(Scale_y) * (item->y - 1), f2fl(Scale_x) * (item->w1 + item->x + item->w2), f2fl(Scale_y) * (item->y + h));

	gr_set_fontcolor(BM_XRGB(28, 28, 28), -1);

	x = item->w1 + item->x + ((item->w2 - w) / 2);

	gr_scale_string(x * f2fl(Scale_x) , item->y * f2fl(Scale_y), Scale_factor, Scale_factor, btext);
}
...
void kc_drawquestion(kc_item *item)
{
	...
	gr_urect((item->w1 + item->x) * f2fl(Scale_x), (item->y - 1) * f2fl(Scale_y), (item->w1 + item->x + item->w2) * f2fl(Scale_x), (item->y + h) * f2fl(Scale_y));

	gr_set_fontcolor(BM_XRGB(28, 28, 28), -1);

	x = item->w1 + item->x + ((item->w2 - w) / 2);

	gr_scale_string(x * f2fl(Scale_x), item->y * f2fl(Scale_y), Scale_factor, Scale_factor, "?");
}
...
void kc_change_joybutton(kc_item * item)
{
	...
	gr_scale_string(0x8000, INFO_Y * f2fl(Scale_y), Scale_factor, Scale_factor, TXT_PRESS_NEW_JBUTTON);
	...
	kc_drawitem(item, 1);
	nm_restore_background(0, INFO_Y * f2fl(Scale_y), 310 * f2fl(Scale_x), grd_curcanv->cv_font->ft_h * f2fl(Scale_y) + 1);
	game_flush_inputs();
}
...
void kc_change_joyaxis(kc_item * item)
{
	...
	gr_scale_string(0x8000, INFO_Y * f2fl(Scale_y), Scale_factor, Scale_factor, TXT_MOVE_NEW_JOY_AXIS);
...
void kc_change_key(kc_item * item)
{
	...
	gr_scale_string(0x8000, INFO_Y * f2fl(Scale_y), Scale_factor, Scale_factor, TXT_PRESS_NEW_KEY);
...
void kc_change_mouseaxis(kc_item * item)
{
	...
	gr_scale_string(0x8000, INFO_Y * f2fl(Scale_y), Scale_factor, Scale_factor, TXT_MOVE_NEW_MSE_AXIS);
...
void kc_change_mousebutton(kc_item * item)
{
	...
	gr_scale_string(0x8000, INFO_Y * f2fl(Scale_y), Scale_factor, Scale_factor, TXT_PRESS_NEW_MBUTTON);
...
void kconfig_sub(kc_item * items,int nitems, char * title)
{
...

	{
		...
		p = strchr( title, '\n' );
		if ( p ) *p = 32;
		gr_scale_string( 0x8000, 8 * f2fl(Scale_y), Scale_factor, Scale_factor, title );
		if ( p ) *p = '\n';
	}
...
//	}
	...
	gr_scale_string( 0x8000, 20 * f2fl(Scale_y), Scale_factor, Scale_factor, TXT_KCONFIG_STRING_1 );
	gr_set_fontcolor( BM_XRGB(28,28,28), -1 );
	if ( items == kc_keyboard )	{
		gr_set_fontcolor( BM_XRGB(31,27,6), -1 );
		gr_setcolor( BM_XRGB(31,27,6) );
		
		for (i = 0; i < f2i(Scale_y); ++i) {
			gr_scanline(98 * f2fl(Scale_x), 106 * f2fl(Scale_x), 42 * f2fl(Scale_y) + i);
			gr_scanline(120 * f2fl(Scale_x), 129 * f2fl(Scale_x) - 1, 42 * f2fl(Scale_y) + i);
		}
		for (i = -1; i < f2i(Scale_y); ++i) {
			for (k = 0; k < f2i(Scale_x); ++k) {
				gr_pixel(98 * f2fl(Scale_x) + k, 43 * f2fl(Scale_y) + i);
				gr_pixel(98 * f2fl(Scale_x) + k, 44 * f2fl(Scale_y) + i);
			}
		}
		for (i = -1; i < f2i(Scale_y); ++i) {
			for (k = 0; k < f2i(Scale_x); ++k) {
				gr_pixel(128 * f2fl(Scale_x) + k, 43 * f2fl(Scale_y) + i);
				gr_pixel(128 * f2fl(Scale_x) + k, 44 * f2fl(Scale_y) + i);
			}
		}
		gr_scale_string( 109 * f2fl(Scale_x), 40 * f2fl(Scale_y), Scale_factor, Scale_factor, "OR" );

		for (i = 0; i < f2i(Scale_y); ++i) {
			gr_scanline(253 * f2fl(Scale_x), 261 * f2fl(Scale_x), 42 * f2fl(Scale_y) + i);
			gr_scanline(274 * f2fl(Scale_x), 284 * f2fl(Scale_x) - 1, 42 * f2fl(Scale_y) + i);
		}
		for (i = -1; i < f2i(Scale_y); ++i) {
			for (k = 0; k < f2i(Scale_x); ++k) {
				gr_pixel(253 * f2fl(Scale_x) + k, 43 * f2fl(Scale_y) + i);
				gr_pixel(253 * f2fl(Scale_x) + k, 44 * f2fl(Scale_y) + i);
			}
		}
		for (i = -1; i < f2i(Scale_y); ++i) {
			for (k = 0; k < f2i(Scale_x); ++k) {
				gr_pixel(283 * f2fl(Scale_x) + k, 43 * f2fl(Scale_y) + i);
				gr_pixel(283 * f2fl(Scale_x) + k, 44 * f2fl(Scale_y) + i);
			}
		}
		gr_scale_string( 264 * f2fl(Scale_x), 40 * f2fl(Scale_y), Scale_factor, Scale_factor, "OR" );

	} if ( items == kc_joystick )	{
		gr_set_fontcolor( BM_XRGB(31,27,6), -1 );
		gr_setcolor( BM_XRGB(31,27,6) );
		for (i = 0; i < f2i(Scale_y); ++i) {
			gr_scanline(18 * f2fl(Scale_x), 135 * f2fl(Scale_x), 37 * f2fl(Scale_y) + i);
			gr_scanline(181 * f2fl(Scale_x), 294 * f2fl(Scale_x), 37 * f2fl(Scale_y) + i);
			gr_scanline(18 * f2fl(Scale_x), 144 * f2fl(Scale_x), (119 + 10) * f2fl(Scale_y) + i);
			gr_scanline(174 * f2fl(Scale_x), 294 * f2fl(Scale_x), (119 + 10) * f2fl(Scale_y) + i);
		}
		gr_scale_string( 0x8000, 35 * f2fl(Scale_y), Scale_factor, Scale_factor, TXT_BUTTONS );
		gr_scale_string( 0x8000,(117+10) * f2fl(Scale_y), Scale_factor, Scale_factor, TXT_AXES );
		gr_set_fontcolor( BM_XRGB(28,28,28), -1 );
		gr_scale_string( 81 * f2fl(Scale_x), 137 * f2fl(Scale_y), Scale_factor, Scale_factor, TXT_AXIS );
		gr_scale_string( 111 * f2fl(Scale_x), 137 * f2fl(Scale_y), Scale_factor, Scale_factor, TXT_INVERT );
		gr_scale_string( 222 * f2fl(Scale_x), 137 * f2fl(Scale_y), Scale_factor, Scale_factor, TXT_AXIS );
		gr_scale_string( 252 * f2fl(Scale_x), 137 * f2fl(Scale_y), Scale_factor, Scale_factor, TXT_INVERT );
	} else if ( items == kc_mouse )	{
		gr_set_fontcolor( BM_XRGB(31,27,6), -1 );
		gr_setcolor( BM_XRGB(31,27,6) );
		for (i = 0; i < f2i(Scale_y); ++i) {
			gr_scanline(18 * f2fl(Scale_x), 135 * f2fl(Scale_x), 37 * f2fl(Scale_y) + i);
			gr_scanline(181 * f2fl(Scale_x), 294 * f2fl(Scale_x), 37 * f2fl(Scale_y) + i);
			gr_scanline(18 * f2fl(Scale_x), 144 * f2fl(Scale_x), (119 + 5) * f2fl(Scale_y) + i);
			gr_scanline(174 * f2fl(Scale_x), 294 * f2fl(Scale_x), (119 + 5) * f2fl(Scale_y) + i);
		}
		gr_scale_string( 0x8000, 35 * f2fl(Scale_y), Scale_factor, Scale_factor, TXT_BUTTONS );
		gr_scale_string( 0x8000,(117+5) * f2fl(Scale_y), Scale_factor, Scale_factor, TXT_AXES );
		gr_set_fontcolor( BM_XRGB(28,28,28), -1 );
		gr_scale_string( 169 * f2fl(Scale_x), 129 * f2fl(Scale_y), Scale_factor, Scale_factor, TXT_AXIS );
		gr_scale_string( 199 * f2fl(Scale_x), 129 * f2fl(Scale_y), Scale_factor, Scale_factor, TXT_INVERT );
	}
	...
	while(1)		{
		...
		case KEY_PAD6:
			...
			break;
		case KEY_ENTER:	
			gr_bm_ubitblt(grd_curscreen->sc_w, grd_curscreen->sc_h, 0, 0, 0, 0, &grd_curscreen->sc_canvas.cv_bitmap, &VR_offscreen_buffer->cv_bitmap);
...
//------------ Read pitch_time -----------
	if ( !slide_on )	{
		...
		if ((use_mouse)&&(Config_control_type==CONTROL_CYBERMAN))	{
			kp += mouse_button_down_time(MB_PITCH_FORWARD, &down_x, &down_y)/(PH_SCALE*2);
			kp -= mouse_button_down_time(MB_PITCH_BACKWARD, &down_x, &down_y)/(PH_SCALE*2);
		}
...
//----------- Read vertical_thrust_time -----------------
	...
	if ( slide_on )	{
		...
		// From Cyberman...
		if ((use_mouse)&&(Config_control_type==CONTROL_CYBERMAN))	{
			Controls.vertical_thrust_time -= mouse_button_down_time(MB_PITCH_FORWARD, &down_x, &down_y);
			Controls.vertical_thrust_time += mouse_button_down_time(MB_PITCH_BACKWARD, &down_x, &down_y);
		}
	...
	// From mouse buttons
	if ( (use_mouse)&&(kc_mouse[8].value < 255 )) Controls.vertical_thrust_time += mouse_button_down_time( kc_mouse[8].value, &down_x, &down_y );
	if ( (use_mouse)&&(kc_mouse[9].value < 255 )) Controls.vertical_thrust_time -= mouse_button_down_time( kc_mouse[9].value, &down_x, &down_y );
	...
	// From Cyberman...
	if ((use_mouse)&&(Config_control_type==CONTROL_CYBERMAN))	{
		Controls.vertical_thrust_time += mouse_button_down_time(MB_Z_UP, &down_x, &down_y)/2;
		Controls.vertical_thrust_time -= mouse_button_down_time(MB_Z_DOWN, &down_x, &down_y)/2;
	}
...
//---------- Read heading_time -----------
	...
	if (!slide_on && !bank_on)	{
		...
		// From Cyberman...
		if ((use_mouse)&&(Config_control_type==CONTROL_CYBERMAN))	{
			kh -= mouse_button_down_time(MB_HEAD_LEFT, &down_x, &down_y)/PH_SCALE;
			kh += mouse_button_down_time(MB_HEAD_RIGHT, &down_x, &down_y)/PH_SCALE;
		}
...
//----------- Read sideways_thrust_time -----------------
	...
	if ( slide_on )	{
		...
		// From cyberman
		if ((use_mouse)&&(Config_control_type==CONTROL_CYBERMAN))	{
			Controls.sideways_thrust_time -= mouse_button_down_time(MB_HEAD_LEFT, &down_x, &down_y);
			Controls.sideways_thrust_time += mouse_button_down_time(MB_HEAD_RIGHT, &down_x, &down_y);
		}
	...
	// From mouse buttons
	if ( (use_mouse)&&(kc_mouse[6].value < 255 )) Controls.sideways_thrust_time -= mouse_button_down_time( kc_mouse[6].value, &down_x, &down_y );
	if ( (use_mouse)&&(kc_mouse[7].value < 255 )) Controls.sideways_thrust_time += mouse_button_down_time( kc_mouse[7].value, &down_x, &down_y );
...
//----------- Read bank_time -----------------
	...
	if ( bank_on )	{
		...
		// From Cyberman...
		if ((use_mouse)&&(Config_control_type==CONTROL_CYBERMAN))	{
			Controls.bank_time -= mouse_button_down_time(MB_HEAD_LEFT, &down_x, &down_y);
			Controls.bank_time += mouse_button_down_time(MB_HEAD_RIGHT, &down_x, &down_y);
		}
	...
	// From keyboard...
	if ( kc_keyboard[20].value < 255 ) Controls.bank_time += speed_factor*key_down_time( kc_keyboard[20].value )/2;
	if ( kc_keyboard[21].value < 255 ) Controls.bank_time += speed_factor*key_down_time( kc_keyboard[21].value )/2;
	if ( kc_keyboard[22].value < 255 ) Controls.bank_time -= speed_factor*key_down_time( kc_keyboard[22].value )/2;
	if ( kc_keyboard[23].value < 255 ) Controls.bank_time -= speed_factor*key_down_time( kc_keyboard[23].value )/2;
	...
	// From mouse buttons
	if ( (use_mouse)&&(kc_mouse[11].value < 255 )) Controls.bank_time += mouse_button_down_time( kc_mouse[11].value, &down_x, &down_y );
	if ( (use_mouse)&&(kc_mouse[12].value < 255 )) Controls.bank_time -= mouse_button_down_time( kc_mouse[12].value, &down_x, &down_y );
	...
	// From Cyberman
	if ((use_mouse)&&(Config_control_type==CONTROL_CYBERMAN))	{
		Controls.bank_time += mouse_button_down_time(MB_BANK_LEFT, &down_x, &down_y);
		Controls.bank_time -= mouse_button_down_time(MB_BANK_RIGHT, &down_x, &down_y);
	}
...
//----------- Read forward_thrust_time -------------
	...
	// From mouse buttons
	if ( (use_mouse)&&(kc_mouse[2].value < 255 )) Controls.forward_thrust_time += mouse_button_down_time( kc_mouse[2].value, &down_x, &down_y );
	if ( (use_mouse)&&(kc_mouse[3].value < 255 )) Controls.forward_thrust_time -= mouse_button_down_time( kc_mouse[3].value, &down_x, &down_y );
...
//----------- Read fire_primary_down_count
	...
	if ((use_mouse)&&(kc_mouse[0].value < 255 )) Controls.fire_primary_down_count += mouse_button_down_count(kc_mouse[0].value, &down_x, &down_y);
...
//----------- Read fire_secondary_down_count
	...
	if ((use_mouse)&&(kc_mouse[1].value < 255 )) Controls.fire_secondary_down_count += mouse_button_down_count(kc_mouse[1].value, &down_x, &down_y);
...
//----------- Read fire_flare_down_count
	...
	if ((use_mouse)&&(kc_mouse[4].value < 255 )) Controls.fire_flare_down_count += mouse_button_down_count(kc_mouse[4].value, &down_x, &down_y);
...
//----------- Read drop_bomb_down_count
	...
	if ((use_mouse)&&(kc_mouse[26].value < 255 )) Controls.drop_bomb_down_count += mouse_button_down_count(kc_mouse[26].value, &down_x, &down_y);
...
//----------- Read rear_view_down_count
	...
	if ((use_mouse)&&(kc_mouse[25].value < 255 )) Controls.rear_view_down_count += mouse_button_down_count(kc_mouse[25].value, &down_x, &down_y);
