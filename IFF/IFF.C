...
int convert_ilbm_to_pbm(iff_bitmap_header *bmheader)
{
	...
	new_data = (ubyte*)malloc((bmheader->w * bmheader->h)*sizeof(byte));
...
//            {ret=IFF_NO_MEM; goto done;}
       //MALLOC(new_data, ushort, bm->bm_w * bm->bm_h * 2);//hack by KRB also a bug I believe. It is allocating twice the needed memory.
		new_data = (ushort*)malloc(bm->bm_w * bm->bm_h * 2);//I left it as previously done, thinking the *2 means sizeof(ushort)
       if (new_data == NULL)
           return IFF_NO_MEM;
...
int write_body(FILE *ofile,iff_bitmap_header *bitmap_header,int compression_on)
{
   ...
   // MALLOC( new_span, ubyte, bitmap_header->w + (bitmap_header->w/128+2)*2);//hack by KRB, also allocating twice the needed memory, probably a bug
	new_span = (ubyte*)malloc(bitmap_header->w+(bitmap_header->w/128+2)*2);//left it alone, as in 2 lines above -KRB
