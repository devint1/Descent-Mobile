...
RetrySelection:
	...
	control_choice = Config_control_type;				// Assume "MOUSE" (since we're iOS)

	// We don't have any control choices for them yet
...
int read_player_file()
{
	...
	//read kconfig data
	{
		if (fread( kconfig_settings, MAX_CONTROLS*CONTROL_MAX_TYPES, 1, file )!=1)
			errno_ret=errno;
		else if (fread(&Config_use_gyroscope, sizeof(ubyte), 1, file ) != 1)
			errno_ret=errno;
		...
		// Adding controller support could change things...
		if (errno_ret==EZERO)	{
			// kc_set_controls();
		}
	}
...
//	}
	...
	//write kconfig info
	{
		if (fwrite( kconfig_settings, MAX_CONTROLS*CONTROL_MAX_TYPES, 1, file )!=1)
			errno_ret=errno;
		else if (fwrite(&Config_use_gyroscope, sizeof(ubyte), 1, file ) != 1)
			errno_ret=errno;
