...
void nm_draw_background1(char * filename)
{
	...
	bmp.bm_data = NULL;
	pcx_error = pcx_read_bitmap(filename,&bmp,BM_LINEAR,NULL);
...
void nm_draw_background(int x1, int y1, int x2, int y2 )
{
	...
	scale_bitmap(&nm_background, scale_vec);
...
void nm_string( bkg * b, int w1,int x, int y, char * s )
{
	...
	gr_scale_string( x, y, Scale_factor, Scale_factor, s );
	...
	if (p && (w1>0) )	{
		...
		gr_scale_string( x+w1-w, y, Scale_factor, Scale_factor, s1 );
...
void nm_rstring( bkg * b,int w1,int x, int y, char * s )
{
	...
	// CHANGED
	gr_bm_bitblt(w1, h + 1, x-w1, y, x-w1, y, b->background, &(grd_curcanv->cv_bitmap) );
	
	gr_scale_string( x-w, y, Scale_factor, Scale_factor, s );
}
...
void update_cursor( newmenu_item *item)
{
	...
	if (time & 0x8000)
		gr_scale_string( x, y, Scale_factor, Scale_factor, CURSOR_STRING );
	else {
		gr_setcolor( BM_XRGB(0,0,0) );
...
void nm_string_inputbox( bkg *b, int w, int x, int y, char * text, int current )
{
	...
	if ( current )	{
		gr_scale_string( x+w1, y, Scale_factor, Scale_factor, CURSOR_STRING );
	}
}
...
//NO_SOUND_PAUSE	}
	...
	if ( title )	{
		...
		tw = string_width * f2fl(Scale_factor);
		th = string_height * f2fl(Scale_factor);
	}
	if ( subtitle )	{
		grd_curcanv->cv_font = SUBTITLE_FONT;
	...
	th += 8 * f2fl(Scale_factor);		//put some space between titles & body
	...
	for (i=0; i<nitems; i++ )	{
		...
		string_width *= f2fl(Scale_factor);
		string_height *= f2fl(Scale_factor);
		average_width *= f2fl(Scale_factor);
		...
		if ( item[i].type == NM_TYPE_INPUT )	{
			...
			string_width *= f2fl(Scale_factor);
			if ( string_width > MAX_TEXT_WIDTH * f2fl(Scale_factor) ) 
				string_width = MAX_TEXT_WIDTH * f2fl(Scale_factor);
			item[i].value = -1;
			shifted_up = true;
		}
...
//	y = (grd_curscreen->sc_h-h)/2;
	...
	w += 30 * f2fl(Scale_factor);
	h += 30 * f2fl(Scale_factor);

	if ( w > grd_curscreen->sc_w ) w = grd_curscreen->sc_w;
	if ( h > grd_curscreen->sc_h ) h = grd_curscreen->sc_h;
...
// ty = 15 + (yborder/4);
	...
	if ( title )	{
		...
		tw = string_width * f2fl(Scale_factor);
		th = string_height * f2fl(Scale_factor);
		gr_scale_printf(0x8000, ty, Scale_factor, Scale_factor, title);
		ty += th;
	}
	...
	if ( subtitle )	{
		...
		tw = string_width * f2fl(Scale_factor);
		th = string_height * f2fl(Scale_factor);
		gr_scale_printf( 0x8000, ty, Scale_factor, Scale_factor, subtitle );
		ty += th;
	}
	...
	for (i=0; i<nitems; i++ )	{
		item[i].x = 15 * f2fl(Scale_factor) + twidth + right_offset;
		item[i].y += 15 * f2fl(Scale_factor);
		if ( item[i].type==NM_TYPE_RADIO )	{
...
ReadFileNames:
	...
	if ((dir = opendir( Document_path )) != NULL) {
		while ((ent = readdir( dir )) != NULL) {
			if (NumFiles<MAX_FILES)	{
				...
				strncpy( &filenames[NumFiles*14], ent->d_name, 13 );
				if ( player_mode )	{
					char * p;
	...
	if (!initialized) {	
		...
		gr_bm_bitblt(VR_offscreen_buffer->cv_bitmap.bm_w, VR_offscreen_buffer->cv_bitmap.bm_h, 0, 0, 0, 0, &(grd_curcanv->cv_bitmap), &(VR_offscreen_buffer->cv_bitmap) );
	...
	while(!done)	{
		...
		// Scaled coordinates
		int rect_x0 = 0.3125 * grd_curscreen->sc_w;
		int rect_x1 = 0.6875 * grd_curscreen->sc_w;
		int rect_y0, rect_y1;
		int string_x = grd_curscreen->sc_w * 0.328125;

		if (ofirst_item != first_item )	{
			gr_setcolor( BM_XRGB( 0,0,0)  );
			for (i=first_item; i<first_item+NumFiles_displayed; i++ )	{
				int w, h, aw, y;
				y = ((i - first_item)*(grd_curscreen->sc_h * 0.06)) + w_y + grd_curscreen->sc_h * 0.225;
				rect_y0 = y - grd_curscreen->sc_h * 0.005;
				rect_y1 = y + grd_curscreen->sc_h * 0.055;
				if ( i >= NumFiles )	{
					gr_setcolor( BM_XRGB(0,0,0));
...
int newmenu_filelist( char * title, char * filespec, char * filename )
{
	...
	if( fhandle = FindFirstFile( filespec, 0, &find ) == INVALID_HANDLE_VALUE )	{
		do	{
			if (NumFiles<MAX_FILES)	{
				strncpy( FilenameText[NumFiles], find.cFileName, 13 );
