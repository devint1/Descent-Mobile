...
char scores_filename[FILENAME_MAX];
...
#endif
	sprintf( scores_filename, "%s/%s", Document_path, SCORES_FILENAME );
	return scores_filename;
}
...
ReshowScores:
	...
	gr_scale_string(0x8000, 15 * f2fl(Scale_y), Scale_factor, Scale_factor, TXT_HIGH_SCORES);

	grd_curcanv->cv_font = Gamefonts[GFONT_SMALL];

	gr_set_fontcolor(BM_XRGB(31, 26, 5), -1);
	gr_scale_string((31 + 33 + XX) * f2fl(Scale_x), (46 + 7 + YY) * f2fl(Scale_y), Scale_factor, Scale_factor, TXT_NAME);
	gr_scale_string((82 + 33 + XX) * f2fl(Scale_x), (46 + 7 + YY) * f2fl(Scale_y), Scale_factor, Scale_factor, TXT_SCORE);
	gr_scale_string((127 + 33 + XX) * f2fl(Scale_x), (46 + 7 + YY) * f2fl(Scale_y), Scale_factor, Scale_factor, TXT_SKILL);
	gr_scale_string((170 + 33 + XX) * f2fl(Scale_x), (46 + 7 + YY) * f2fl(Scale_y), Scale_factor, Scale_factor, TXT_LEVELS);
	//	gr_string( 202, 46, "Kills" );
	//	gr_string( 234, 46, "Rescues" );
	gr_scale_string((288 - 42 + XX) * f2fl(Scale_x), (46 + 7 + YY) * f2fl(Scale_y), Scale_factor, Scale_factor, TXT_TIME);

	if (citem < 0)
		gr_scale_string(0x8000, 175 * f2fl(Scale_y), Scale_factor, Scale_factor, TXT_PRESS_CTRL_R);

	gr_set_fontcolor(BM_XRGB(28, 28, 28), -1);

	gr_scale_printf(0x8000, 31 * f2fl(Scale_y), Scale_factor, Scale_factor, "%c%s%c  - %s", 34, Scores.cool_saying, 34, Scores.stats[0].name);
	...
	while (!done)	{
		...
		for (i = 0; i<3; i++)
			if (mouse_button_down_count(i, &mouse_x, &mouse_y)>0) done = 1;
