...
int do_laser_firing(int objnum, int weapon_num, int level, int flags, int nfires)
{
	...
	Laser_offset = ((F1_0*2)*(drand()%10))/10;
	...
	if (objp->id == SMART_ID) {
		...
		if (numobjs == 0) {
			...
			for (i=0; i<NUM_SMART_CHILDREN; i++) {
				if (parent_type == OBJ_PLAYER) {
					int	hobjnum;
					hobjnum = create_homing_missile(objp, objlist[(drand() * numobjs) >> 15].objnum, PLAYER_SMART_HOMING_ID, make_sound);
					// mprintf((0, "Object #%i is a PLAYER smart blob.\n", hobjnum));
				} else {
					int	hobjnum;
					hobjnum = create_homing_missile(objp, objlist[(drand() * numobjs) >> 15].objnum, ROBOT_SMART_HOMING_ID, make_sound);
					// mprintf((0, "Object #%i is a robot smart blob.\n", hobjnum));
				}
				make_sound = 0;
			}
