...
void do_endlevel_flythrough(int n)
{
	...
	if (flydata->first_time || obj->segnum != old_player_seg) {		//moved into new seg
		...
		if (seg_time) {
			flydata->angstep.x = fmax(-MAX_ANGSTEP,fmin(MAX_ANGSTEP,fixdiv(delta_ang(flydata->angles.p,dest_angles.p),seg_time)));
			flydata->angstep.z = fmax(-MAX_ANGSTEP,fmin(MAX_ANGSTEP,fixdiv(delta_ang(flydata->angles.b,dest_angles.b),seg_time)));
			flydata->angstep.y = fmax(-MAX_ANGSTEP,fmin(MAX_ANGSTEP,fixdiv(delta_ang(flydata->angles.h,dest_angles.h),seg_time)));
