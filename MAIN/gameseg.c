...
int trace_segs(vms_vector *p0,int oldsegnum,int recursion_count)
{
	...
	if (recursion_count >= 1024) {		//if no debugging, we'll get past assert
	...
	if (centermask == 0)		//we're in the old segment
		...
		do {
			...
			if (biggest_side != -1) {
				...
				check = trace_segs(p0, seg->children[biggest_side], recursion_count+1);	//trace into adjacent segment
