...
int descent_main(int argc,char **argv)
{
	...
	((void *) descent_critical_error_handler );
...
//		printf( "\n");
	...
	if ( FindArg( "-vfx" ) || (Config_vr_type==1) )	{
		...
		// New stuff for arbitrary width/height
		if (screen_width && !screen_height) {
			printf("Error: Width provided without specifying height\n");
			exit(1);
		}
		if (!screen_width && screen_height) {
...
//	}
	...
	if ((t=gr_init( VR_render_width, VR_render_height ))!=0)
		Error(TXT_CANT_INIT_GFX,t);

	// Needs to be done after graphics so we can get center position
	if (!FindArg("-nomouse")) {
		if (Inferno_verbose) printf("\n%s", TXT_VERBOSE_4);
