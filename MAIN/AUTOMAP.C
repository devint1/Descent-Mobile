...
grs_canvas *print_to_canvas(char *s,grs_font *font, int fc, int bc)
{
	...
	temp_canv = gr_create_canvas(font->ft_w*strlen(s)*f2fl(scale_factor),font->ft_h*2*f2fl(scale_factor));

	gr_set_current_canvas(temp_canv);
	gr_set_curfont(font);
	gr_clear_canvas(255);						//trans color
	gr_set_fontcolor(fc,bc);
	gr_scale_printf(0,0,scale_factor,scale_factor,s);
	...
	for (y=temp_canv->cv_bitmap.bm_h/2;y--;) {
		memcpy(data+(rs*y),data+(rs*y),temp_canv->cv_bitmap.bm_w);
		memcpy(data+(rs*(y)),data+(rs*y),temp_canv->cv_bitmap.bm_w);
	}
...
void do_automap( int key_code )	{
	...
	// Don't seem to need this, all it does is get the screen stuck on black sometimes
	//gr_palette_clear();

	w = VR_render_buffer[0].cv_bitmap.bm_w;
	h = VR_render_buffer[0].cv_bitmap.bm_h;
	...
	// Have to scale the bitmap since it is meant for Mode X
	grs_point scale_vec[] = {
		{ 0,0 },
		{ i2f(320), i2f(400) },
		{ i2f(VR_render_width), i2f(VR_render_height) }
	};
	scale_bitmap(&Automap_background, scale_vec);

	gr_set_curfont(Gamefonts[GFONT_BIG_1]);
	gr_set_fontcolor(BM_XRGB(20, 20, 20), -1);
	gr_scale_printf(w * 0.125, h * 0.055, scale_factor, scale_factor, TXT_AUTOMAP);
	gr_set_curfont(Gamefonts[GFONT_SMALL]);
	gr_set_fontcolor(BM_XRGB(20, 20, 20), -1);
	gr_scale_printf(w * 0.21875, h * 0.88, scale_factor, scale_factor, TXT_TURN_SHIP);
	gr_scale_printf(w * 0.21875, h * 0.92, scale_factor, scale_factor, TXT_SLIDE_UPDOWN);
	gr_scale_printf(w * 0.21875, h * 0.96, scale_factor, scale_factor, TXT_VIEWING_DISTANCE);
