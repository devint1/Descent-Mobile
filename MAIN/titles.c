...
int local_key_inkey(void)
{
	...
	// Simulate enter key for touches
	if ( mouse_button_down_count(0, NULL, NULL) ) {
		rval = KEY_ENTER;
	}
...
int show_title_screen( char * filename, int allow_keys )	
{
	...
	while (1)	{
		if ( (mouse_button_down_count(0, NULL, NULL) > 0) && allow_keys ) break;
...
byte	Door_div_count=0, Animating_bitmap_type=0;
char	Door_dir = 1;
...
void show_bitmap_frame(void)
{
	...
	if (Bitmap_name[0] != 0) {
		...
		switch (Animating_bitmap_type) {
			case 0:	bitmap_canv = gr_create_sub_canvas(grd_curcanv, f2fl(Scale_x) * 220, f2fl(Scale_y) * 45, f2fl(Scale_x) * 64, f2fl(Scale_y) * 64);	break;
			case 1:	bitmap_canv = gr_create_sub_canvas(grd_curcanv, f2fl(Scale_x) * 220, f2fl(Scale_y) * 45, f2fl(Scale_x) * 94, f2fl(Scale_y) * 94);	break;	//	Adam: Change here for your new animating bitmap thing. 94, 94 are bitmap size.
		...
		grs_point scale_pts[] = {
			{ 0, 0 },
			{ i2f(bitmap_ptr->bm_w), i2f(bitmap_ptr->bm_h) },
			{ Scale_factor * bitmap_ptr->bm_w, Scale_factor * bitmap_ptr->bm_h }
		};

		scale_bitmap(bitmap_ptr,scale_pts);
...
void show_briefing_bitmap(grs_bitmap *bmp)
{
	...
	grs_point scale_pts[] = {
		{ 0, 0 },
		{ i2f(bmp->bm_w), i2f(bmp->bm_h) },
		{ Scale_factor * bmp->bm_w, Scale_factor * bmp->bm_h }
	};

	bitmap_canv = gr_create_sub_canvas(grd_curcanv, f2fl(Scale_x) * 220, f2fl(Scale_y) * 45, f2fl(Scale_x) * 166, f2fl(Scale_y) * 138);
	curcanv_save = grd_curcanv;
	grd_curcanv = bitmap_canv;
...
void init_spinning_robot(void)
{
	...
	Robot_canv = gr_create_sub_canvas(grd_curcanv, f2fl(Scale_x) * 138, f2fl(Scale_y) * 55, f2fl(Scale_x) * 166, f2fl(Scale_y) * 138);
}
...
int show_char_delay(char the_char, int delay, int robot_num, int cursor_flag)
{
	...
	Erase_color = gr_gpixel(&grd_curcanv->cv_bitmap, Briefing_text_x, Briefing_text_y);
	...
	if (cursor_flag && delay) {
		...
		gr_scale_printf(Briefing_text_x+1, Briefing_text_y, Scale_factor, Scale_factor, "_" );
	}
	...
	gr_scale_printf(Briefing_text_x, Briefing_text_y, Scale_factor, Scale_factor, message );

	gr_set_fontcolor(Briefing_foreground_colors[Current_color], -1);
	gr_scale_printf(Briefing_text_x+1, Briefing_text_y, Scale_factor, Scale_factor, message );
	...
	while (!done) {
		...
		if (ch == '$') {
			...
			if (ch == 'C') {
				...
				tab_stop = f2fl(Scale_x) * get_message_num(&message);
...
//				prev_ch = 10;							//	read to eoln
			...
			if (prev_ch != '\\') {
				prev_ch = ch;
				Briefing_text_y += 8 * f2fl(Scale_y);
				Briefing_text_x = bsp->text_ulx;
				if (Briefing_text_y > bsp->text_uly + f2fl(Scale_y) * bsp->text_height) {
		...
		if (Briefing_text_x > bsp->text_ulx + f2fl(Scale_x) * bsp->text_width) {
			Briefing_text_x = bsp->text_ulx;
			Briefing_text_y += 8 * f2fl(Scale_y);
		}

		if ((new_page) || (Briefing_text_y > bsp->text_uly + f2fl(Scale_y) * bsp->text_height)) {
			fix	start_time = 0;
