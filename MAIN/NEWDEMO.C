...
void newdemo_start_playback(char * filename)
{
	...
	if (filename==NULL) {
		...
		rnd_demo = 1;
		if( fhandle = FindFirstFile( "*.DEM", &find ) != INVALID_HANDLE_VALUE )	{
			do	{
				NumFiles++;
		...
		NumFiles = 0;
		if( fhandle = FindFirstFile( "*.DEM", _A_NORMAL, &find ) == INVALID_HANDLE_VALUE )	{
			do	{
...
void newdemo_strip_frames(char *outname, int bytes_to_strip)
{
	...
	bytes_done = 0;
	fseek(fileno(infile), 0L, SEEK_END);
	total_size = ftell(fileno(infile));
	fseek(fileno(infile), 0L, SEEK_SET);
